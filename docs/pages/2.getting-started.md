...
To get smaller bundle size, you can use our optional babel plugin to automatically rewrite imports so that only the modules you actually use are imported. Add `react-native-paper/babel` to the `plugins` section in your `.babelrc`:
...
Note: This only works if you are using ES2015 import statements and not for `require`.
